...
global nu
nu = -0.1; % 1;  % 
example = 2; % 1 or 2   % example number
...
for z= 0%:5
    ...
    if example == 1
        T = 100;  % [sec] Process time
    else
        T = 0.6;  %1.75;  %   [sec] Process time
    end
    len_T = 101;
...
%     savePDF(['Ex_',num2str(example),'_PSD'])
    ...
    loglog(x,N_ana(i,:),'k-','markersize',16,'linewidth',2.5)
    ylim([1e-10,1])
...
%    plot(T3,N3*x','+-','markersize',12)
   ...
   % title('Total mass')
    legend({'HC','HCNP','CFV','Exact'},'fontsize',18)
    xlim([0 T])
     ylim([0.998 1.003])
    xlabel('Time (t)','fontsize',25);
